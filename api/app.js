...
// ---------- ถ้า runtime คุณเป็น Node < 18 ให้เปิดสองบรรทัดนี้ และติดตั้ง node-fetch@3 ----------
...
/** อนุญาตเฉพาะโดเมนเว็บ static ของคุณ + localhost สำหรับ dev */
const allowOrigins = [
...
const MONGO_URL = process.env.MONGO_URL || ''; // ว่างได้ (โหมดไม่มี DB)
...
/** Mongo (ทำให้เป็น optional — ไม่มี MONGO_URL ก็ทำงานต่อได้) */
let db = null;
if (MONGO_URL && MONGO_URL.startsWith('mongodb')) {
  MongoClient.connect(MONGO_URL)
    ...
    .catch(err => {
      ...
      // ไม่ปิดโปรเซส เพื่อให้ live endpoints ใช้งานได้
    });
} else {
...
/** ช่วยเทสต์เร็ว ๆ */
app.get('/', (_req, res) => res.send('web-weather API is running'));
app.get('/api/health', (_req, res) => res.json({ ok: true, ts: new Date().toISOString() }));
app.get('/api/meta', (_req, res) => res.json({ place: PLACE, tz: TZ, now: new Date().toISOString() }));

/** เครื่องมือเรียก Open-Meteo */
...
app.get('/api/current', async (_req, res) => {
  try {
    ...
    return res.json({
      time_utc: toISO(c.time || Date.now()),
...
app.get('/api/readings/recent', async (req, res) => {
  try {
    ...
    if (!time?.length) return res.json([]);
...
app.get('/api/daily', async (req, res) => {
  try {
    ...
    // fallback (provider)
    const j = await callOpenMeteo('daily=temperature_2m_max,temperature_2m_min,precipitation_sum&forecast_days=14');
    const { time, temperature_2m_min, temperature_2m_max, precipitation_sum } = j.daily || {};
    const out = [];
    for (let i = 0; i < Math.min(days, time?.length || 0); i++) {
      out.push({
        date: time[i],
